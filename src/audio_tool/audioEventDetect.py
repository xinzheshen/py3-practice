...
config = {
    ...
    'source_path':        r'D:\cats\audios\chimes\615_715\7000_615_7000_not_mix.wav',
    # 'source_path':        r'D:\cats\audios\chimes\615_715\audio_with_615.wav',
    ...
    'matching_samples':   r'D:\cats\audios\chimes\615_715\samples',
...
sample_rate = 16000
sample_crop_start = 3 # The first 4 seem to get damaged
sample_crop_end = 2
...
def get_pcm_data(file, sample_rate=16000):
    devnull = open(os.devnull)
    proc = subprocess.Popen(['ffmpeg', '-i', file, '-f', 's16le', '-ac', '1', '-ar', str(sample_rate), '-'], stdout=subprocess.PIPE, stderr=devnull)
    devnull.close()
...
def stft_raw(series, sample_rate, win_length, hop_length, hz_count, dtype, n_fft=2048):
    window = 'hann'
    pad_mode='reflect'
...
def extract_feature_and_plt_by_py_audio_analysis(audio_path):
    [freq_sample, x] = audioBasicIO.readAudioFile(audio_path)
    frames, f_names = audioFeatureExtraction.stFeatureExtraction(x, freq_sample, 0.050 * freq_sample,
                                                                 0.025 * freq_sample)
    data = empty_feature_data()
    data["name"] = audio_path.split(os.path.sep)[-1].split(".")[0]
    data["y_labels"] = f_names
    data["frame"] = frames
    print("End")
...
def get_samples_data():
    ...
    for sample_path in files:
        if os.path.isfile(sample_path):
            sample_path = uniform_audio(sample_path)
            sample_rate, signal = scipy.io.wavfile.read(sample_path)
            sample_series = scale * signal
...
def uniform_audio(file):
    audio_wav = AudioSegment.from_wav(file)
    loudness = audio_wav.dBFS
    volume_diff = config["required_volume"] - loudness
    changed = audio_wav + volume_diff

    # changed = audio_wav.set_frame_rate(config["frame_rate"])
    # changed = changed + volume_diff
    path_tmp = os.path.split(file)
    output_folder = os.path.join(path_tmp[0], uniform)
    if not os.path.exists(output_folder):
        os.mkdir(output_folder)
    output = os.path.join(output_folder, path_tmp[1])
...
if __name__ == '__main__':
    ...
    config['source_path'] = uniform_audio(config['source_path'])
    scale = 1./float(1 << ((8 * 2) - 1))
    sample_rate_source, signal = scipy.io.wavfile.read(config['source_path'])
    source_series = scale * signal
    # todo verify sample rate equal
    ...
    for block_start in range(config['source_frame_start'], config['source_frame_end'], n_columns): # Time in 31 blocks
        ...
        set_data = abs((scipy.fftpack.rfft(fft_window * source_frames[:, block_start:block_end], axis=0)).astype(dtype))
